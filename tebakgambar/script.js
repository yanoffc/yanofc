const questions = [
  ...
  },
  {
    ...
    function displayQuestion() {
        ...
        questionImage.alt = currentQuestion.jawaban;
    }

    function checkAnswer() {
        const answerInput = document.getElementById('answerInput').value.toUpperCase();
        const currentQuestionIndex = usedIndexes[usedIndexes.length - 1];
        const currentQuestion = questions[currentQuestionIndex];
        if (answerInput === currentQuestion.jawaban) {
            correctCount++;
            alert('✅ Jawaban Benar!');
        } else {
            wrongCount++;
    ...
    function updateScoreDisplay() {
        scoreDisplay.textContent = `✅ = ${correctCount} - ❌ = ${wrongCount}`;
    }
